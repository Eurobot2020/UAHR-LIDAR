...
def cb_recognized_objects(msg):
    for n,p_object in enumerate(msg.array):
        if(p_object.dist):
            print(f"El objeto para triangular {n} esta en {p_object.angle} grados a una distancia de {p_object.dist}")        
        else:
...
def cb_enemy_robots(msg):
    for p_enemy in msg.array:
        print(f"Puede que haya un enemigo en {p_enemy.angle} grados a una distancia de {p_enemy.dist}")
